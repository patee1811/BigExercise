...
class ThreatsObject : public BaseObject // ke thua
{
public:
    ThreatsObject();
    ~ThreatsObject();
    ...
    void InitThreats();
    void set_type_move(const int& TypeMove) {type_move_ = TypeMove;} // kieu di chuyen cho threat
    void SetAnimationPos(const int& pos_a, const int& pos_b) {animation_a_ = pos_a; animation_b_ = pos_b;}
    void set_input_left(const int& ipLeft) {input_type_.left_ = ipLeft;};
    void ImpMoveType(SDL_Renderer* screen); // xu li di chuyen
    ...
    void InitBullet(BulletObject* p_bullet, SDL_Renderer* screen); //khoi tao thong so bullet
    void MakeBullet(SDL_Renderer* screen, const int& x_limit, const int& y_limit);
    void RemoveBullet(const int& idx);

private:
    int map_x_;    //gioi han ban do
    int map_y_;
